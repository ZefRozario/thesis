...
\label{chap:PCBTextrap}
\chapterquote{I'm all energy, no technique.}%
{Dr. Dwayne Spiteri, referring to his squash ability.}
...
\begin{equation*}
    \epsilon_{b}= \frac{\# \text{of jets that are $b$-tagged} }{\text{Total \# of truth $b$ jets}}.
\end{equation*}
...
As we see in Figures~\ref{fig:track}, the relative tracking uncertainties, the contributions to the overall extrapolation tracking uncertainty are minimal, peaking at $\approx$ 0.03 at 3 TeV. The tracking-related uncertainties are as follows: \verb|TRK_FAKE_RATE_LOOSE_TIDE| and \verb|TRK_FAKE_RATE_LOOSE| account for uncertainties in the fake track rate in data and simulation, with TIDE referring to a dedicated tuning for dense environments. The remaining uncertainties correspond to different sources of uncertainty in the track reconstruction efficiency for tracks passing loose quality criteria. \verb|TRK_EFF_LOOSE_TIDE| and \verb|TRK_EFF_LOOSE_PP0| stem from data-driven corrections and pileup modelling in the efficiency determination. \verb|TRK_EFF_LOOSE_PHYSMODEL| addresses uncertainties from the underlying physics modelling used in simulation. \verb|TRK_EFF_LOOSE_GLOBAL| represents a global uncertainty in track reconstruction efficiency applied across the detector. Finally, \verb|TRK_EFF_LOOSE_IBL| captures potential miss-modelling of the IBL, which is particularly relevant for high-$p_T$ tracks near the beamline. These components are combined in quadrature to form the total tracking uncertainty.
\begin{figure}[htbp]
    % \centering
...
Compared to the systematics described in Sections~\ref{sec:trackunc} and~\ref{sec:jetcalunc} the modelling uncertainties are considerably larger as seen in Figure~\ref{fig:model}. With the main contributions coming from the parton shower and B-hadron interactions. When these are propagated in to the extrapolation uncertainty these significantly inflate the overall value at any $p_T$ bin, peaking at 0.36 at 3 TeV. The modelling uncertainties are: \verb|FSR| corresponds to the uncertainty from final-state radiation modelling and is typically assessed by varying the QCD radiation parameters or using alternative parton shower configurations. \verb|parton shower| represents uncertainties associated with different choices of parton shower generators (e.g., \textsc{Pythia} vs \textsc{Herwig}), which affect the development of the jet and the kinematics of the b-hadron. \verb|B-hadron interaction| accounts for mismodelling of b-hadron decays and their interactions with the detector material, which can influence the reconstruction of secondary vertices and impact parameter significance. The \verb|Total| uncertainty is the quadrature sum of all modelling components.

% \begin{figure}[htbp]
%     \centering
%     \includegraphics[scale=0.15]{figures/App_btag_highpt_extrap/modelling_important_uncs.png}
%     \caption{The decomposed modelling uncertainty containing: Final state radiation (FSR, light grey), B-hadron interaction (yellow), parton shower (dark grey) and the total combination of all modelling systematics (green). }
%         \label{fig:model}
% \end{figure}
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[scale=0.15]{figures/App_btag_highpt_extrap/modelling_important_extrap_uncs.png}
%     \caption{The modelling extrapolation uncertainty containing: Final state radiation (FSR, light grey), B-hadron interaction (yellow), parton shower (dark grey) and the total combination of all modelling systematics (green).}
...
\begin{figure}[htbp]
    ...
    % \begin{subfigure}[t]{0.9\textwidth}
        ...
        \caption{Decomposed modelling uncertainty: FSR (light grey), B-hadron interaction (yellow), parton shower (dark grey), and total (green) for PFlow $b$-jets with DL1r at 60\% efficiency.}
        \label{fig:model}
    % \end{subfigure}
    ...
    %     \caption{modelling extrapolation uncertainty with the same components.}
    %     \label{fig:modelextrap}
    % \end{subfigure}
    % \caption{Relative(top) and extrapolated (bottom) modelling uncertainties for Pflow $b$-jets with DL1r at 60\% efficiency.}
    % \label{ffig:model}
\end{figure}
\subsection{Global Uncertainty}
As discussed in Section~\ref{sec:model} the modelling uncertainties are the dominant contributors for the extrapolation uncertainty, this is confirmed in Figure~\ref{fig:globalunc}.
\begin{figure}[htbp]
    % \centering
    % \begin{subfigure}[t]{0.9\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/App_btag_highpt_extrap/global_uncs.png}
        \caption{Nominal global uncertainty: modelling (green), tracking (orange), jet calibration (blue), and total (black) for PFlow $b$-jets with DL1r at 60\% efficiency.}
        \label{fig:globalunc}
    % \end{subfigure}
...
After some discussion, with the internal flavour tagging group of ATLAS (FTAG), the number of EVs to keep after truncation were settled on per flavour and are shown in table~\ref{table:EV}. The question remains: if the number of EVs retained contain enough information to effectively encapsulate uncertainties evaluated thus far.
\begin{center}
\begin{table}[h!]
